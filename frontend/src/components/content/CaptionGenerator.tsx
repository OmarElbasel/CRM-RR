      ...
      <div className="col-span-12 lg:col-span-5 space-y-6">
        <div className="bg-white p-6 rounded shadow-sm border border-slate-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-xs font-black text-ds-text-2 uppercase tracking-wider mb-2">Product Description</label>
              <textarea 
        ...
        <div className="bg-ds-accent/20/10 border border-secondary/20 p-5 rounded-xl flex gap-4 transition-all hover:bg-ds-accent/20/20 cursor-default">
          ...
          <div>
            ...
            <p className="text-xs text-on-ds-accent/20/80 leading-relaxed font-body">Add specific keywords like &quot;limited edition&quot; or &quot;free delivery&quot; to increase conversion by 24%.</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm flex flex-col h-full min-h-[500px]">
          <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 uppercase tracking-widest leading-none">
            ...
            <div className="flex gap-2">
              ...
              >
                <RefreshCw className="text-sm" /> Try Again
              </button>
            </div>
          </div>
            ...
            {result && (
              <>
                <div className="relative group">
                  ...
                  <p className={`text-lg text-ds-text leading-relaxed font-body ${lang === 'ar' ? 'text-right' : ''}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                    {lang === 'ar' ? result.caption_ar : result.caption_en}
                  </p>
                </div>

                <div className="space-y-4 pt-4">
                  <span className="text-[10px] font-black text-ds-text-2 uppercase tracking-widest block">Recommended Hashtags</span>
                  <div className={`flex flex-wrap gap-2 ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
                    {(lang === 'ar' ? result.hashtags_ar : result.hashtags_en).map((tag, i) => (
                      <button 
                        key={i} 
                        onClick={() => copyToClipboard(tag)}
                        className="bg-paper px-3 py-1.5 rounded-full text-[10px] font-bold text-ds-text-2 border border-ds-line-2 hover:bg-primary/10 hover:text-primary transition-all flex items-center gap-1.5 group"
                      >
                        {tag} <Copy className="text-[14px] opacity-0 group-hover:opacity-100 transition-opacity" />
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <div className="mt-auto border-t border-slate-100 p-6 flex justify-between items-center bg-slate-50/30">
            <div className="flex gap-4">
              ...
              >
                <Copy className="text-lg" /> Copy All
              </button>
              <button 
                disabled={!result}
              ...
              >
                <Share2 className="text-lg" /> Post Directly
              </button>
            </div>
            {result && (
