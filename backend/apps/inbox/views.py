...
class ConnectView(APIView):
    ...
    @extend_schema(operation_id='channel_connect', tags=['Channels'], responses={302: None})
...
class CallbackView(APIView):
    ...
    @extend_schema(operation_id='channel_callback', tags=['Channels'], responses={302: None})
    def get(self, request):
        ...
        try:
            ...
            logger.error('Token exchange failed for org=%s platform=%s: %s', request.org.pk, platform_lower, exc)
            return Response(
                {
        ...
        token_expires_at = timezone.now() + timedelta(seconds=expires_in) - timedelta(days=3)
        ...
        return redirect(f'{settings.FRONTEND_URL}/settings/channels?connected={platform_lower}')
...
class ChannelListView(APIView):
    ...
    @extend_schema(operation_id='channel_list', tags=['Channels'], responses={200: ChannelSerializer(many=True)})
    def get(self, request):
    ...
    def _process_event(self, event: dict, platform: str, page_id: str):
        ...
        if not sender_id and 'value' in event:
            ...
            if messages:
                ...
                text = wa_msg.get('text', {}).get('body', '') if wa_msg.get('type') == 'text' else ''
        ...
        if platform == 'WHATSAPP':
            channel = SocialChannel.objects.filter(
                phone_number_id=page_id, platform=platform, is_active=True,
            ).first()
        else:
            channel = SocialChannel.objects.filter(
                page_id=page_id, platform=platform, is_active=True,
            ).first()

        if not channel:
            logger.warning('No active channel found for platform=%s page_id=%s', platform, page_id)
            return
        ...
        try:
            ...
            logger.info('Duplicate message ignored: platform=%s msg_id=%s', platform, msg_id)
            return

        if created:
            # Publish new_message event immediately (before AI processing)
            from .tasks import _publish_redis_event
            _publish_redis_event(channel.org.pk, 'new_message', {
                'message_id': msg.pk,
...
class InboxListView(APIView):
    ...
    def get(self, request):
        ...
        if unread_only == 'true':
            qs = qs.filter(messages__read=False, messages__direction='INBOUND').distinct()
...
class ReplyView(APIView):
    ...
    @extend_schema(operation_id='message_reply', tags=['Inbox'], request=ReplySerializer, responses={201: MessageSerializer})
    ...
    def post(self, request, message_id):
        ...
        outbound = Message.objects.create(
            ...
            platform_msg_id=f'out_{int(time.time() * 1000)}_{original.pk}',
            direction='OUTBOUND',
            content=reply_text,
            read=True,
            sent_at=timezone.now(),
...
class DisconnectView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.warning('Token revocation failed for channel=%s: %s', channel.pk, exc)
