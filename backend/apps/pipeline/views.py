...
class PipelineBoardView(APIView):
    ...
    def get(self, request):
        ...
        # Order by score descending within each stage
        qs = qs.order_by('-ai_score', '-updated_at')

        # Group by stage
        stages = []
        aggregate_total = Decimal('0.00')
        for stage_key, stage_label in BOARD_STAGES:
            stage_deals = [d for d in qs if d.stage == stage_key]
            total_value = sum(
                (d.value or Decimal('0.00')) for d in stage_deals
            )
...
class DealCreateView(APIView):
    ...
    @extend_schema(operation_id='deal_create', tags=['Pipeline'], request=DealCreateSerializer)
    def post(self, request):
        org = request.org
        serializer = DealCreateSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(
                {'error': 'Invalid deal data.', 'error_ar': 'بيانات الصفقة غير صالحة.', 'details': serializer.errors},
...
class DealDetailView(APIView):
    ...
    def get(self, request, deal_id):
        org = request.org
        try:
            deal = Deal.objects.select_related('contact').prefetch_related(
                'tasks', 'notifications',
    ...
    @extend_schema(operation_id='deal_update', tags=['Pipeline'], request=DealUpdateSerializer)
    def patch(self, request, deal_id):
        ...
        if not serializer.is_valid():
            return Response(
                {'error': 'Invalid update data.', 'error_ar': 'بيانات التحديث غير صالحة.', 'details': serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class DealTaskCreateView(APIView):
    ...
    @extend_schema(operation_id='deal_task_create', tags=['Pipeline'], request=DealTaskCreateSerializer)
    def post(self, request, deal_id):
        ...
        if not serializer.is_valid():
            return Response(
                {'error': 'Invalid task data.', 'error_ar': 'بيانات المهمة غير صالحة.', 'details': serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class DealTaskUpdateView(APIView):
    ...
    @extend_schema(operation_id='deal_task_update', tags=['Pipeline'], request=DealTaskUpdateSerializer)
...
class NotificationListView(APIView):
    ...
    def get(self, request):
        org = request.org
        notifications = PipelineNotification.objects.filter(
            org=org,
        ).select_related('deal').order_by('-created_at')[:50]

        unread_count = PipelineNotification.objects.filter(
            org=org, read_at__isnull=True,
        ).count()
