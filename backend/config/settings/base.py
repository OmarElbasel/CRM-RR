...
SPECTACULAR_SETTINGS = {
    ...
    "TAGS": [
        ...
        {
            "name": "Billing",
            "description": "Stripe billing, plan management, and API key rotation",
        },
        {
            "name": "Inbox",
            "description": "Unified social inbox — conversations, messages, and AI processing",
        },
        {
            "name": "Channels",
            "description": "Social channel OAuth connection, disconnect, and management",
        },
        {
            "name": "Pipeline",
            "description": "CRM pipeline — deals, tasks, and notifications",
        },
        {
            "name": "Shopify & Orders",
            "description": "Shopify store integration, order management, and abandoned cart recovery",
        },
        {
            "name": "Content",
            "description": "AI-powered bilingual social media content generation",
        },
        {
            "name": "Scheduler",
            "description": "Post scheduler and WhatsApp broadcast",
        },
    ],
}
...
FEATURE_FLAGS = {
    ...
    "SHOPIFY_ORDER_HUB": env.bool("FLAG_SHOPIFY_ORDER_HUB", default=False),
    "TIKTOK_INBOX": env.bool("FLAG_TIKTOK_INBOX", default=False),
    "CONTENT_ASSISTANT": env.bool("FLAG_CONTENT_ASSISTANT", default=False),
    "POST_SCHEDULER": env.bool("FLAG_POST_SCHEDULER", default=False),
    # Phase 10 — Launch Hardening
    "RATE_LIMITING": env.bool("FLAG_RATE_LIMITING", default=False),
...
CELERY_BEAT_SCHEDULE = {
    ...
    "sync-tiktok-comments": {
        "task": "apps.inbox.tasks.sync_tiktok_comments",
        "schedule": crontab(minute="*/30"),
    },
    "publish-scheduled-posts": {
        "task": "apps.scheduler.tasks.publish_scheduled_posts",
        "schedule": crontab(),
    },
    # Phase 10 — Launch Hardening
    "detect-abuse": {
        "task": "apps.orgs.tasks.detect_abuse",
        "schedule": crontab(minute=0),  # hourly
    },
    "send-weekly-digest": {
        "task": "apps.billing.tasks.send_weekly_digest_all_orgs",
...
# Phase 10 — Transactional Emails (Resend)
RESEND_API_KEY = env("RESEND_API_KEY", default="")
RESEND_FROM_EMAIL = env("RESEND_FROM_EMAIL", default="noreply@rawaj.app")

# Phase 10 — Rate Limiting (uses existing Redis cache backend)
